...
int main(int argc, char *argv[])
{
	...
	for(l=0;l<MAX_LINES;l++) lines[l]=NULL;
	for(l=0;l<MAX_TOKENS;l++) t[l]=NULL;
	for(l=0;l<MAX_QUESTIONS;l++) labels[l]=NULL;
	
	if (argc != 2) return 0; /* Make sure we have the right number of arguments */

	/* Open the file */
	fp = fopen(argv[1],"r");
	if (fp == NULL) return 0; /* Make sure that file was opened properly */
	...
	while(x<i)
	{
		...
		if (lines[x][0] == '?') /* Look for questions */
		{
			...
			for(l=0;l<q;l++) {free(labels[l]); labels[l]=NULL;} /* Free the labels */
		}
	...
	for(l=0;lines[l];l++) {free(lines[l]); lines[l]=NULL;}
	for(l=0;labels[l];l++) {free(labels[l]); labels[l]=NULL;}
	for(l=0;t[l];l++) {free(t[l]); t[l]=NULL;}
	free(line); line=NULL;

	if (fp) fclose(fp); /* Close the file */
